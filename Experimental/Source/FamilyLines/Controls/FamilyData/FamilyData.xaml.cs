...
namespace KBS.FamilyLines
{
    public partial class FamilyData : System.Windows.Controls.UserControl
    {
		...
		// KBR 03/12/2012 Code to 'rebuild' the controls. The family has been changed, either during
		// construction, or on load/import. Set up bindings etc.
        private void Rebuild()
        {
            ...
            App.Family.ContentChanged += new EventHandler<KBS.FamilyLinesLib.ContentChangedEventArgs>(OnFamilyContentChanged);
            ...
            BirthdaysControl.PeopleCollection = family;
        ...
        private static PeopleCollection family;

        public FamilyData()
        {
            InitializeComponent();
            family = App.Family; // KBR 03/12/2012 use a local copy; the global will change on load/import
            Rebuild();
        ...
        {
            ListCollectionView view = new ListCollectionView(family);
        ...
        public static bool TagCloudFilter(object o)
        {
            ...
            return (p != null && !string.IsNullOrEmpty(p.LastName));
        }
        ...
        {
            Person p = o as Person;
            return (p != null && p.AgeGroup != AgeGroup.Unknown);
        }
        ...
        public void Refresh()
        {
		    // KBR 03/12/2012 check to see if the family has been changed (from load/import). NOTE: if the 'new' family has the 
			// same number of people as the 'old' family, this doesn't work!!!
            if (family.Count != App.Family.Count)
            {
        ...
        private void UpdateColumnWidths()
        {
            ...
            CalculateColumnWidth(PrefixMenu, Suffix);
            CalculateColumnWidth(AgeMenu, Age);
            CalculateColumnWidth(TreeMenu, TreeNum);
        ...
        private void UpdateColumnsSettings()
        {
            ...
            Properties.Settings.Default.ColumnPrefix = PrefixMenu.IsChecked;
            Properties.Settings.Default.ColumnTreeNum = TreeMenu.IsChecked;
        ...
        private void LoadColumnsSettings()
        {
            ...
            PrefixMenu.IsChecked = Properties.Settings.Default.ColumnPrefix;
            TreeMenu.IsChecked = Properties.Settings.Default.ColumnTreeNum;
        }
        ...
        private void FilterTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            if (FamilyEditor.Items.Count > 0)
            {
                ...
                {
                    // KBR 03/10/2012 HACK Not sure why an exception occurs on the 2d and subsequent
                    // rebuilds of the FamilyData views ...
                }
            }
