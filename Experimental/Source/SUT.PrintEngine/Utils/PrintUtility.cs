...
namespace SUT.PrintEngine.Utils
{
    public class PrintUtility
    {
        ...
        {
            var fixed_name = Path.GetInvalidFileNameChars().Aggregate(printerFullName, (current, c) => current.Replace(c.ToString(), "_"));
            return String.Format("{0}\\{1}_printTicket.xml", Constants.Print.SETTINGS_FOLDER, fixed_name);
        }
        ...
        {
            var loc = GetSaveLocation(printerFullName);
            if (File.Exists(loc))
            {
                var fileStream = new FileStream(loc, FileMode.Open);
