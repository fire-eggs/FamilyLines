    ...
    public class GedcomImport
    {
        ...
        private void UpdatePeopleIDs()
        {
            foreach (Person p in people)
            {
                ...
                // KBR 03/10/2012 performance sink during import. Do this on Save and initial access
        ...
        private static string GetPrefix(XmlNode node)
        {
        ...
        {
            return GetValue(node, "NAME/NSFX");
        }
        ...
        private static DateTime? GetValueDate(XmlNode node, string xpath)
        {
            ...
            try
            {
                ...
                // Strip 'Abt', 'Aft', 'Bef', 'c.', 'c'
                value = value.Replace("abt", "");
                ...
                // KBR 03/30/2012 this breaks date formats "November 3, 1951"
                // truncate to first comma or slash
                //int dex = value.IndexOf(',');
                //if ( dex < 0 )
                //    dex = value.IndexOf('/');
                //if (dex > 0)
                //    value = value.Substring(0, dex);
				
                //look for quarter abbreviations and remove
                value = value.Replace("jan-feb-mar", "jan");
                value = value.Replace("apr-may-jun", "apr");
                value = value.Replace("jul-aug-sep", "jul");
                value = value.Replace("oct-nov-dec", "oct");
                ...
                // wipe spaces for next test
                value = value.Trim();

                // dateparse won't handle year by itself - assuming a 4 char value is a year
                if (value.Length == 4)
                    value = "1 jan " + value;

                if (!string.IsNullOrEmpty(value))
                {
                    // KBR take first crack at "November 3, 1951"
                    //                    result = DateTime.Parse(value, CultureInfo.InvariantCulture);
                    DateTime val;
                    var ok = DateTime.TryParse(value, out val);
                    if (ok) 
                        return val;

                    // KBR then deal with things like "19 jun 1951, age 54"
                    int dex = value.IndexOf(',');
                    if (dex < 0)
                        dex = value.IndexOf('/');
                    if (dex > 0)
                    ...
                    // dateparse won't handle "mon year" by itself - assuming a letter at start indicates a month
                    if (Char.IsLetter(value[0]))
                        value = "1 " + value;
                    ok = DateTime.TryParse(value, out val);
                    if (ok) result = val;
                }
            }
            catch
