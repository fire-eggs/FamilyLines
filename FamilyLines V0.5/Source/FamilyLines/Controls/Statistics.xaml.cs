    ...
    public partial class Statistics
    {
        ...
        public void DisplayStats(PeopleCollection people, SourceCollection sources, RepositoryCollection repositories)
        {
            ...
            foreach (Person p in people)
            {
                ...
                foreach (var gedFact in p.Facts)
                {
                    if (gedFact != null)
                    {
                        if (!string.IsNullOrEmpty(gedFact.Source) &&
                            !string.IsNullOrEmpty(gedFact.Citation) &&
                            !string.IsNullOrEmpty(gedFact.Description))
                            citations++;
                    }
                }
                foreach (var gedEvent in p.Events)
                {
                    if (gedEvent != null)
                    {
                        // TODO make a 'HasCitation' property
                        if (!string.IsNullOrEmpty(gedEvent.Source) &&
                            !string.IsNullOrEmpty(gedEvent.Citation) &&
                            !string.IsNullOrEmpty(gedEvent.Place) ||
                            (gedEvent.Date != null && gedEvent.Date.DateTime1 > DateTime.MinValue))
                            citations++;
                    }
                }
                ...
                foreach(Relationship rel in p.Relationships)
                {
                    ...
                    {
                        var spouseRel = ((SpouseRelationship)rel);
